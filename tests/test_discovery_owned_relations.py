        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            with sqlite3.connect(index) as db:
                ...
                db.executemany('INSERT INTO series_lookup VALUES (?, ?, ?, ?, ?, ?)', [
                    (1, 'manga', native, m._title_key(native), f'["{title}"]', ''),
                    (2, 'manga', related_native, m._title_key(related_native), '[]', ''),
                ])
            ...
            class Gateway:
                def fetch_all(self, *_):
                    return [{'id': 10, 'library_id': 2, 'content_kind': 'manga',
            ...
            target = {'id': 10, 'series_name': title, 'series_alias': None,
                      'localized_series': localized, 'library_id': 2, 'content_kind': 'manga'}
            with patch.object(m, '_series_type_matches_library', return_value=True):
